...
const std::string BKR::BKR_FILE = "data/BKR.txt";
...
BKR::BKR (const Data & d)
{
	...
	size_t first = d.get_name().rfind('/');
	if (first == std::string::npos) first = 0; else ++first;
	size_t length = d.get_name().rfind('.');
	if (length != std::string::npos) length -= first;
	std::string instance(d.get_name().substr(first, length));
